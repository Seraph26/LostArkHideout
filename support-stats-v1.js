...
function unflatten(data){const values=typeof data==='string'?JSON.parse(data):data,hydrated=new Array(values.length),seen=new Set();function get(i){if(i===-1)return undefined;if(i<0||i>=values.length)return undefined;if(hydrated[i]!==undefined)return hydrated[i];if(seen.has(i))return hydrated[i];seen.add(i);const v=values[i];if(v===null||typeof v==='string'||typeof v==='number'||typeof v==='boolean')return hydrated[i]=v;if(Array.isArray(v)){const a=[];hydrated[i]=a;for(const x of v)a.push(typeof x==='number'?get(x):x);return a}if(v&&typeof v==='object'){const o={};hydrated[i]=o;for(const[k,x]of Object.entries(v))o[k]=typeof x==='number'?get(x):x;return o}return v}return get(0)}
...
function collect(root){const out=[];const seen=new WeakSet();function walk(v,ctx=''){if(!v||typeof v!=='object'||seen.has(v))return;seen.add(v);if(Array.isArray(v)){for(const x of v)walk(x,ctx);return}const cls=classNorm(v.class||v.className||v.supportClass||v.name);const med=Number(v.median);if(cls&&Number.isFinite(med)){out.push({class:cls,median:med,category:ctx||''})}for(const[k,x]of Object.entries(v)){const next=categoryFrom('',k)||categoryFrom(x&&typeof x==='string'?x:'',k)||ctx;walk(x,next)}}walk(root);return out}
...
async function fetchStats(enc){const key=JSON.stringify({id:enc?.id,boss:enc?.boss,difficulty:enc?.difficulty,patch:enc?.patch,minIlvl:enc?.minIlvl,maxIlvl:enc?.maxIlvl});if(CACHE.has(key))return CACHE.get(key);const p=payloadFor(enc);const r=await fetch(`${WORKER}?payload=${encodeURIComponent(p)}`,{cache:'no-store'});if(!r.ok)throw Error(`Bible raid stats HTTP ${r.status}`);const raw=await r.json();const root=unflatten(raw?.data);const records=collect(root);const stats=merge(records);const value={ok:true,key,stats,rawCount:records.length,updatedAt:new Date().toISOString()};CACHE.set(key,value);window.__LOSTARK_SUPPORT_STATS__=value;return value}
