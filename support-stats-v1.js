/* Lost Ark Party — dynamic Bible support uptime data */
(()=>{
'use strict';
...
const ALIASES={
 ...
 'armoche-g1':['Brelshaza, Ember in the Ashes','Covetous Master Echidna','Mistress of Desire Echidna','Armoche Gate 1'],
...
function collect(root){const out=[];const seen=new WeakSet();function walk(v,ctx=''){if(!v||typeof v!=='object'||seen.has(v))return;seen.add(v);if(Array.isArray(v)){for(const x of v)walk(x,ctx);return}const cls=classNorm(v.class||v.className||v.supportClass||v.supportClassName||v.roleClass||v.name);const med=Number(v.median??v.med??v.value);const ownCategory=categoryFrom(v.stat,'stat')||categoryFrom(v.spec,'spec')||ctx;if(cls&&Number.isFinite(med))out.push({class:cls,median:med,category:ownCategory||''});for(const[k,x]of Object.entries(v)){const next=categoryFrom('',k)||categoryFrom(x&&typeof x==='string'?x:'',k)||ownCategory;walk(x,next)}}walk(root);return out}
...
async function fetchCandidate(enc,boss){const p=payloadFor(enc,boss);const r=await fetch(`${WORKER}?payload=${encodeURIComponent(p)}`,{cache:'no-store'});if(!r.ok)throw Error(`Bible raid stats HTTP ${r.status}`);const raw=await r.json();const root=unflatten(raw?.data??raw);const records=collect(root);const stats=merge(records);return {stats,rawCount:records.length,boss};}
async function fetchStats(enc){if(!enc)return null;const key=JSON.stringify({id:enc?.id,boss:enc?.boss,difficulty:enc?.difficulty,patch:enc?.patch,minIlvl:enc?.minIlvl,maxIlvl:enc?.maxIlvl});if(CACHE.has(key)){window.__LOSTARK_SUPPORT_STATS__=CACHE.get(key);return CACHE.get(key)}
 const candidates=[...(ALIASES[enc.id]||[]),enc.boss].filter(Boolean).filter((v,i,a)=>a.indexOf(v)===i);let lastError=null;for(const boss of candidates){try{const result=await fetchCandidate(enc,boss);if(hasSupportData(result.stats)){const value={ok:true,key,stats:result.stats,rawCount:result.rawCount,resolvedBoss:boss,updatedAt:new Date().toISOString()};CACHE.set(key,value);window.__LOSTARK_SUPPORT_STATS__=value;return value;}}catch(e){lastError=e}}
 if(lastError)throw lastError;const empty={ok:false,key,stats:merge([]),rawCount:0,resolvedBoss:null,updatedAt:new Date().toISOString()};CACHE.set(key,empty);window.__LOSTARK_SUPPORT_STATS__=empty;return empty;}
...
const tick=()=>{try{ensure()}catch{}};
setTimeout(tick,250);setTimeout(tick,750);setTimeout(tick,1500);setTimeout(tick,3000);setInterval(tick,2000);
})();
