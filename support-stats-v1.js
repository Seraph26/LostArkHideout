...
function responseRoots(raw){
 const roots=[];
 const add=v=>{if(v===undefined||v===null)return;roots.push(v)};
 const data=raw?.data;
 if(Array.isArray(data)){
  // Bible has returned both a compact reference array directly and an envelope
  // whose first entry is the compact payload. Try both shapes without assuming
  // which one a future endpoint version will use.
  if(Array.isArray(data[0]))add(data[0]);
  if(typeof data[0]==='string')add(data[0]);
  add(data);
 }else if(data!==undefined)add(data);
 if(raw?.result!==undefined)add(raw.result);
 add(raw);
 return roots;
}
function decodeRecords(raw){
 const all=[];const seenRoots=new Set();
 for(const candidate of responseRoots(raw)){
  let signature='';try{signature=JSON.stringify(candidate)}catch{}
  if(signature&&seenRoots.has(signature))continue;
  if(signature)seenRoots.add(signature);
  try{all.push(...collect(unflatten(candidate)))}catch{}
 }
 return all;
}
