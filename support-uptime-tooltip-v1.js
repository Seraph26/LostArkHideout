...
const LABELS={damage:'Damage',defenseReduction:'Defense Reduction',crit:'Critical Rate',critDamage:'Critical Damage',attackPower:'Attack Power',attackSpeed:'Attack Speed',moveSpeed:'Move Speed',mana:'Mana',supportAmplification:'Support Amplification',identity:'Identity',positionalGeneral:'Positional Damage',positionalSpecific:'Positional Damage'};
function canonical(v){const raw=clean(v),k=raw.replace(/[._-]+/g,' ').toLowerCase().replace(/\s+/g,'');return LABELS[k]||LABELS[raw.toLowerCase()]||raw}
function normalizeSummary(el){
  const text=clean(el.textContent);if(!/^Synergies\s*:/i.test(text))return;
  ...
  const parts=body.split(/\s*,\s*/).map(canonical).filter(Boolean);
  body=parts.join(', ');
  const uptime=body.match(/^(.*?)(?:\s*[·•]\s*Support uptime\s*([\d.]+)%)\s*$/i);
  if(uptime)body=`${uptime[1].trim()} · Support uptime ${uptime[2]}%`;
  const desired=`Synergies: ${body}`;if(clean(el.textContent)===desired)return;el.textContent=desired;
}
function wrap(){document.querySelectorAll('#suggestedParties .party-synergies').forEach(el=>{normalizeSummary(el);if(el.querySelector('.support-uptime-label'))return;const walker=document.createTreeWalker(el,NodeFilter.SHOW_TEXT);const nodes=[];while(walker.nextNode())nodes.push(walker.currentNode);for(const node of nodes){const text=node.nodeValue||'',match=/Support uptime/i.exec(text);if(!match)continue;const before=text.slice(0,match.index),after=text.slice(match.index+match[0].length),frag=document.createDocumentFragment();if(before)frag.appendChild(document.createTextNode(before));const span=document.createElement('span');span.className='support-uptime-label';span.textContent='Support uptime';span.setAttribute('data-tooltip',tooltipText);span.setAttribute('aria-label',tooltipText);span.setAttribute('role','note');frag.appendChild(span);if(after)frag.appendChild(document.createTextNode(after));node.parentNode.replaceChild(frag,node);break}})}
