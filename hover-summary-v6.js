/* Lost Ark Hideout — compact hover summary authority v9 */
(()=>{
'use strict';
...
function parseRows(m){
 ...
 for(const row of c.querySelectorAll('.chb-synergy')){
  ...
  // Raid optimizer v17 stores the same contribution data in a shorter detail form:
  // "supportAmplification from Bailsxo: 814 · observed median 100.00%".
  // Recover the percentage from the recipient's base CP so the compact renderer can
  // use exactly the same contribution format as General Optimization.
  if(raidMode()){
   x=t.match(/^(.+?)\s+from\s+(.+?):\s*([+-]?[\d,]+(?:\.\d+)?)(?:\s*[·-]\s*observed\s+median\s+([\d.]+)%?)?/i);
   if(x){
    const value=num(x[3]);
    if(value>0){
     out.push({effect:clean(x[1]),source:clean(x[2]),value,pct:base?value/base*100:0,uptime:Number.isFinite(Number(x[4]))?Number(x[4]):null});
    }
   }
  }
 }
...
function renderDps(m){const c=card(m);if(!c||role(m)!=='dps')return;const rows=parseRows(m);if(!rows.length)return;const supportNames=new Set(members().filter(x=>role(x)==='support').map(name));const groups=group(rows,'source');const synergyPct=rows.filter(r=>!supportNames.has(r.source)).reduce((a,r)=>a+r.pct,0);const supportPct=rows.filter(r=>supportNames.has(r.source)).reduce((a,r)=>a+r.pct,0);const stats=c.querySelector('.chb-stats');if(stats)stats.innerHTML=`<span>Party Synergy +${synergyPct.toFixed(2)}%</span><span>Support Impact +${supportPct.toFixed(2)}%</span>`;let html='';if(raidMode()){const label=encounterLabel();if(label)html+=`<div class="chb-compact-encounter">${esc(label)}<br>Support compatibility uses encounter data.</div>`}html+=groups.map(g=>`<div class="chb-summary-row">+${g.value.toLocaleString(undefined,{maximumFractionDigits:2})} estimated ${supportNames.has(g.name)?'support':'synergy'} contribution from ${esc(g.name)} - ${g.pct.toFixed(2)}% of base power</div>`).join('');replaceDetails(c,html)}
function renderSupport(m){const c=card(m);if(!c||role(m)!=='support')return;const party=m.closest('.party');const ps=party?[...party.querySelectorAll('.party-member')]:members();const rows=[];for(const dps of ps){if(dps===m||role(dps)!=='dps')continue;for(const r of parseRows(dps))if(r.source===name(m))rows.push({target:name(dps),value:r.value,pct:r.pct,uptime:r.uptime})}if(!rows.length)return;const groups=group(rows,'target'),total=groups.reduce((a,g)=>a+g.value,0),base=cp(m);const stats=c.querySelector('.chb-stats');if(stats)stats.innerHTML=`<span>Party Synergy +0.00%</span><span>Support Impact +${(base?total/base*100:0).toFixed(2)}%</span>`;let html='';if(raidMode()){const label=encounterLabel();if(label)html+=`<div class="chb-compact-encounter">${esc(label)}<br>Support compatibility uses encounter data.</div>`}html+=groups.map(g=>`<div class="chb-summary-row">+${g.value.toLocaleString(undefined,{maximumFractionDigits:2})} estimated support contribution to ${esc(g.name)} - ${g.pct.toFixed(2)}% of ${esc(g.name)}'s base power</div>`).join('');replaceDetails(c,html)}
...
function start(){window.LostArkHoverSummaryV1={active:true,version:9};css();apply();const root=document.getElementById('suggestedParties')||document.body;let timer=0;const schedule=()=>{clearTimeout(timer);timer=setTimeout(apply,0)};new MutationObserver(schedule).observe(root,{childList:true,subtree:true,characterData:true});document.getElementById('optimizeBtn')?.addEventListener('click',()=>[0,20,50,100,200,400,800,1500,3000].forEach(ms=>setTimeout(apply,ms)),true);document.getElementById('raidSpecificSelect')?.addEventListener('change',()=>setTimeout(apply,0),true);[50,150,300,600,1000,2000,4000].forEach(ms=>setTimeout(apply,ms))}
