...
function encounterHtml(m){if(!raidMode())return '';const label=encounterLabel();if(!label)return '';/* Raid Specific adds Encounter Favorability directly under the compatibility
   line, where the reader is already looking at encounter context. */
let extra='';try{extra=window.LostArkHoverExtras?.(m?.dataset?.characterId||'')||''}catch{}
return `<div class="chb-compact-encounter">${esc(label)}<br>Support compatibility uses encounter data.${extra}</div>`}
