...
function specFor(p,b){
 ...
 /* The specialization comes from Bible's Ark Passive Enlightenment nodes, and
    ...
    false positives, because Bible embeds a payload naming every spec a class
    can take: Haylebrella, whose nodes read "T1 Drizzle" and whose rendered page
    never contains the words "Wind Fury", was labelled Wind Fury from markup
    alone. Right for all seven characters checked, where the flat search got
    that one wrong.

    So when the nodes are present they are the whole answer: if none of them
    matches a known spec, return nothing and let the card show the class name,
    rather than inventing one from elsewhere on the page.

    The flat search survives only for build profiles cached before the nodes
