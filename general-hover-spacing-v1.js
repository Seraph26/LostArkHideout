...
function formatSynergySummary(){
 document.querySelectorAll('#suggestedParties .party-synergies').forEach(el=>{
  const existing=el.querySelector('.support-uptime-label');
  if(existing)return;
  ...
  for(const node of nodes){
   ...
   const span=document.createElement('span');span.className='support-uptime-label';span.textContent='Support Uptime';span.setAttribute('data-tooltip',synergyTitle);span.setAttribute('aria-label',synergyTitle);span.setAttribute('role','note');frag.appendChild(span);
...
function css(){let s=document.getElementById('general-hover-spacing-v1-style');if(!s){s=document.createElement('style');s.id='general-hover-spacing-v1-style';document.head.appendChild(s)}s.textContent=`
...
`}
function tooltip(){let t=document.getElementById('lah-support-uptime-tooltip');if(!t){t=document.createElement('div');t.id='lah-support-uptime-tooltip';t.textContent=synergyTitle;document.body.appendChild(t)}return t}
function showTooltip(el){const t=tooltip();t.textContent=el.getAttribute('data-tooltip')||synergyTitle;t.style.display='block';const r=el.getBoundingClientRect(),tw=t.offsetWidth,th=t.offsetHeight;let left=r.left+(r.width/2)-(tw/2),top=r.top-th-10;left=Math.max(16,Math.min(left,window.innerWidth-tw-16));if(top<8)top=r.bottom+10;t.style.left=Math.round(left)+'px';t.style.top=Math.round(top)+'px'}
function hideTooltip(){const t=document.getElementById('lah-support-uptime-tooltip');if(t)t.style.display='none'}
function start(){css();const run=()=>{formatSynergySummary();formatCards()};run();const root=document.getElementById('suggestedParties')||document.body;let timer;const schedule=()=>{clearTimeout(timer);timer=setTimeout(run,30)};new MutationObserver(schedule).observe(root,{childList:true,subtree:true,characterData:true});document.addEventListener('mouseover',e=>{const u=e.target.closest?.('#suggestedParties .support-uptime-label');if(u){showTooltip(u);return}if(e.target.closest?.('#suggestedParties .party-member')||e.target.closest?.('#suggestedParties .party-synergies'))schedule()},true);document.addEventListener('mouseout',e=>{if(e.target.closest?.('#suggestedParties .support-uptime-label'))hideTooltip()},true);window.addEventListener('scroll',hideTooltip,{passive:true});window.addEventListener('resize',hideTooltip);[0,50,150,300,600,1000,2000,4000,8000].forEach(ms=>setTimeout(run,ms));setInterval(run,1000)}
