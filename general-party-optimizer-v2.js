...
function member(c,s,p){const i=info(c),hover=hoverHtml(c,s,p);return`<div class="party-member authoritative-member" draggable="true" data-character-id="${esc(c.id)}"><a class="party-character-link" href="${esc(i.url)}" target="_blank" rel="noopener noreferrer">${esc(i.name)}</a><span class="party-class-label">${esc(i.cls)}</span><span class="party-role-label ${i.role==='Support'?'support':'dps'}">${i.role}</span><span class="party-stat-label">${esc(pos(c))} · CP ${Math.round(i.cp).toLocaleString()}</span>${hover}</div>`}
...
window.LostArkGeneralModel={score,hoverHtml,info,role,resolve};
