...
function role(c){const p=profile(c);const o=p.roleOverride;if(o==='Support'||o==='DPS')return o;/* pinned role wins: a support class can legitimately be specced DPS */
...
function render(a,after=false){const h=document.querySelector('#suggestedParties');if(!h)return;const{p1,p2}=resolve(a),s1=score(p1),s2=score(p2),total=(s1?.total||0)+(s2?.total||0);const box=(n,p,s,k)=>`<article class="party authoritative-party"><div class="party-heading"><div><h3>${n}</h3><div class="party-score">Estimated potential: <strong>${Math.round(s?.total||0).toLocaleString()}</strong></div></div><div class="party-meta">${p.length}/4 · ${p.filter(x=>role(x)==='Support').length} support</div></div><div class="party-dropzone authoritative-dropzone" data-drop-party="${k}">${p.map(c=>member(c,s,p)).join('')}</div><div class="party-synergies"><strong>Synergies:</strong> ${partySynergyLabels(p)||'None'}${s?.coherence?` · Support uptime ${s.coherence}%`:''}</div></article>`;h.innerHTML=`<div class="authoritative-summary"><strong>Combined estimated potential: ${Math.round(total).toLocaleString()}</strong><span> — one General Optimization scoring model; character CP is never modified.</span></div>${encounterGuidance()}${box('Party 1',p1,s1,'party1')}${box('Party 2',p2,s2,'party2')}${after?`<div class="swap-comparison"><strong>Manual party change</strong>${bestSwapHtml(p1,p2)}</div>`:''}<div class="optimizer-result"><strong>${after?'Party arrangement updated.':'General Optimization complete.'}</strong> ${after?'Best available swap is calculated against the current arrangement using the same scoring model.':'The optimizer exhaustively evaluated all valid 4/4 partitions.'}</div>`}
