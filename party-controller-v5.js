...
// THE TOP ROSTER IS THE SINGLE SOURCE OF TRUTH.
// We do not detect, infer, canonicalize, or hardcode a class here.
// For every party member, find the exact character card in #roster and copy
// the already-rendered name, class and icon from that card.
function rosterData(c){
  const cards=[...document.querySelectorAll('#roster article.character')];
  const targetUrl=String(c?.url||'');
  const targetName=String(c?.profile?.name||c?.name||'').trim();
  for(const card of cards){
    const link=card.querySelector('a.character-bible-link[href]');
    if(!link)continue;
    const sameUrl=targetUrl && link.href===new URL(targetUrl,location.href).href;
    const sameName=targetName && link.textContent.trim()===targetName;
...
function displayData(c){
  const r=rosterData(c);
  if(r)return r;
  // This is only a rendering fallback while the top card is still loading.
  return {name:c?.profile?.name||c?.name||'',class:'',icon:c?.profile?.classIcon||'',role:''};
}
function role(c){const d=displayData(c);return ['Bard','Paladin','Artist','Valkyrie'].includes(d.class)?'Support':'DPS'}
...
function install(){const ob=$('#optimizeBtn');if(ob)ob.onclick=()=>{const a=chars();if(a.length){save(bestAssignment(a));render()}};const z=$('#suggestedParties');if(!z)return;let timer=0;const rerender=()=>{clearTimeout(timer);timer=setTimeout(render,50)};const roster=$('#roster');if(roster)new MutationObserver(rerender).observe(roster,{subtree:true,childList:true,characterData:true,attributes:true});new MutationObserver(()=>{}).observe(z,{childList:true});setTimeout(render,500);setTimeout(render,1500);setTimeout(render,3000)}
