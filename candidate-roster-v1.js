...
/* This used to overwrite the Main Group key with the New Additions so that
   BuildProfilesV3.refresh() -- which read that key -- would fetch their builds,
   then restore it afterwards. Any interruption inside that window (a reload, a
   closed tab, a rejected promise) left the Main Group permanently replaced by
   the New Additions. Importing a share link was a reliable trigger, since a
   recipient has no cached builds and the swap always ran. refresh() now takes
   the list directly, so the stored roster is never touched. */
function refreshCandidateBuilds(){if(buildRefreshRunning)return;const refresh=window.LostArkBuildProfilesV3?.refresh;if(typeof refresh!=='function')return;const ns=newChars().filter(c=>c?.url);if(!ns.length)return;const cache=read('lostark-hideout-build-profiles-v3',{}),missing=ns.some(c=>{const u=String(c.url).replace(/\/$/,'');return !cache[c.url]&&!cache[u]&&!Object.keys(cache).some(k=>String(k).replace(/\/$/,'')===u)});if(!missing)return;buildRefreshRunning=true;Promise.resolve(refresh(ns)).then(()=>{buildRefreshRunning=false;renderNew()}).catch(()=>{buildRefreshRunning=false})}
...
function allCharacters(){
 const list=[...(mainState().characters||[]),...normalizeNew()].filter(c=>c&&c.id);
 /* An imported lobby replaces the roster wholesale. Inert unless the stored
    source is "live", and inert entirely when the module is absent. */
 return window.LostArkLiveGroup?window.LostArkLiveGroup.resolveRoster(list):list;
}
