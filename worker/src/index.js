...
function isValidBibleCharacterUrl(value) {
  try {
    ...
    if (url.hostname !== BIBLE_HOST) return false;

    const parts = url.pathname.split("/").filter(Boolean);

    if (parts.length < 3) return false;
    if (parts[0].toLowerCase() !== "character") return false;

    if (
      url.pathname.toLowerCase().includes("roster") ||
      url.pathname.toLowerCase().includes("siblings") ||
      url.pathname.toLowerCase().includes("account")
    ) {
      return false;
    }
  ...
  async fetch(request, env, ctx) {
    ...
    if (requestUrl.pathname === "/visits") {
      try {
        ...
        return json(
          { ok: false, error: "Unable to update visit counter." },
          500,
          origin
        );
      }
      ...
      return json(
        {
          ok: false,
          error:
            "Use /character?url=https://lostark.bible/character/REGION/NAME",
        },
        404,
        origin
      );
    ...
    if (!bibleUrl) {
      return json({ ok: false, error: "Missing character URL." }, 400, origin);
    }
      ...
      return json(
        {
          ok: false,
          error:
            "Invalid Bible character URL. Only specific /character/... URLs are accepted.",
        },
        400,
        origin
      );
    ...
    try {
      const response = await fetch(bibleUrl, {
        ...
        headers: {
          ...
          "User-Agent":
            "Mozilla/5.0 (compatible; LostArkHideout/1.0; +https://github.com/Seraph26/LostArkHideout)",
        },
        cf: {
          cacheTtl: 0,
      ...
      if (!response.ok) {
        return json(
          {
            ok: false,
            error: `Bible returned HTTP ${response.status}.`,
      ...
      return json(
        {
          ok: false,
          error: "Unable to retrieve the Bible character page.",
          character: characterName,
          region,
        },
        502,
