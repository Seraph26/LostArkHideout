...
/* Switching roster source or importing a lobby changes what render() should
   draw, but nothing else can ask it to redraw. Exposed so those paths do not
   have to reload the page. */
window.LostArkDashboard={render:()=>render()};
...
function render(){
 /* The three summary cards counted state.characters directly, so a live lobby
    left them showing the Main Group's numbers while the roster below showed the
    lobby. Read through the same source the roster does. */
 const onLive=!!(window.LostArkLiveGroup&&window.LostArkLiveGroup.isLive&&window.LostArkLiveGroup.isLive()
   &&window.LostArkLiveGroup.characters().length);
 const shown=onLive?window.LostArkLiveGroup.characters():state.characters;
 const liveMeta=onLive?window.LostArkLiveGroup.meta():null;
 const complete=shown.filter(c=>c.profile),il=complete.map(c=>c.profile.ilvl).filter(Number.isFinite),cp=complete.map(c=>c.profile.cp).filter(Number.isFinite);$('#playerCount').textContent=`${shown.length} / ${liveMeta&&liveMeta.players?liveMeta.players:MAX_CHARACTERS}`;$('#avgIlvl').textContent=il.length?Math.round(il.reduce((a,b)=>a+b,0)/il.length):'—';$('#avgCp').textContent=cp.length?Math.round(cp.reduce((a,b)=>a+b,0)/cp.length).toLocaleString():'—';$('#dataMode').textContent='Bible profiles';$('#rosterNote').textContent='Only explicitly supplied character URLs are retrieved. Raid CP priority: Estimated Raid Loadout → Current Loadout (Raid). Chaos Dungeon Loadout is never used.';$('#roster').innerHTML=shown.length?shown.map(c=>{const p=c.profile||{},cls=correctedClassForCharacter(c),name=p.name||c.name,icon=iconForProfile(p);return`<article class="character"><div class="character-head"><div><h3 class="character-title">${icon?`<img class="class-icon" src="${icon}" alt="${escapeHtml(cls)}">`:''}<a class="character-bible-link" href="${escapeHtml(c.url)}" target="_blank" rel="noopener noreferrer">${escapeHtml(name)}</a></h3><div class="class">${escapeHtml(cls)}</div></div>${liveMeta?'':`<button class="remove-character" data-id="${escapeHtml(c.id)}" type="button">Remove</button>`}</div><div class="stats"><div class="stat">iLvl<b>${formatNumber(p.ilvl)}</b></div><div class="stat">CP<b>${formatNumber(p.cp)}</b></div></div><div class="privacy-note">${p?`Bible profile loaded · ${escapeHtml(p.loadout)} · CP source: ${escapeHtml(p.cpSource)}`:escapeHtml(c.profileError||'Profile pending')}</div></article>`}).join(''):(liveMeta?'<div class="empty-roster">This lobby has no characters.</div>':'<div class="empty-roster">No designated main characters have been added.</div>');document.querySelectorAll('.remove-character').forEach(b=>b.addEventListener('click',()=>removeCharacter(b.dataset.id)));renderSuggestions()}
