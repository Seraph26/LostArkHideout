(() => {
  ...
  const ICON_FILES = {
    ...
    Deathblade:'ClassIcon-Assassin-Deathblade.png', Shadowhunter:'ClassIcon-Assassin-Shadowhunter.png', Reaper:'ClassIcon-Assassin-Reaper.png',
  ...
  const isSouleaterSvg = svg => /M524\.5,356\.5/.test(String(svg || '')) || /M524\.5%2C356\.5/.test(String(svg || ''));
  const iconUrl = name => {
    if (String(name || '').replace(/\s+/g,'').toLowerCase() === 'souleater') return `data:image/svg+xml;charset=utf-8,${encodeURIComponent(SOULEATER_SVG)}`;
    return ICON_FILES[name] ? `https://lostark.fandom.com/wiki/Special:Redirect/file/${encodeURIComponent(ICON_FILES[name])}` : '';
  };

  window.LostArkHideoutClassData = { CLASS_ID_TO_NAME, ICON_FILES, canonical, iconUrl, isSouleaterSvg, SOULEATER_SVG };
})();
