(() => {
  ...
  // Authoritative Souleater icon supplied from the Bible roster page.
  ...
  function findClassId(text) {
    ...
    const raid = [...text.matchAll(/classification\s*:\s*["']raid_merged["']/gi)];
    for (const hit of raid) {
      const chunk = text.slice(hit.index, hit.index + 20000);
      const m = chunk.match(/classId\s*:\s*["']([a-z_]+)["']/i);
      if (m) { const c = canonical(m[1]); if (c) return c; }
    }
    const ids = [...text.matchAll(/classId\s*:\s*["']([a-z_]+)["']/gi)];
    for (const m of ids) { const c = canonical(m[1]); if (c) return c; }
    return null;
  }
  ...
  window.LostArkHideoutClassData = { CLASS_ID_TO_NAME, ICON_FILES, canonical, iconUrl, findClassId, SOULEATER_SVG };
})();
