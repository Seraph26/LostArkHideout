...
(() => {
  ...
  function normalize(chars, existing) {
    ...
    for (const id of existing.party1 || []) {
      if (valid.has(id) && !seen.has(id) && p1.length < MAX_PER_PARTY) { p1.push(id); seen.add(id); }
    }
    for (const id of existing.party2 || []) {
      if (valid.has(id) && !seen.has(id) && p2.length < MAX_PER_PARTY) { p2.push(id); seen.add(id); }
    }
  ...
  function showImpact(before, after, label) {
    ...
    el.innerHTML = `<strong>${esc(label)}: ${change >= 0 ? '+' : ''}${change.toFixed(2)}%</strong> combined estimated potential damage (${format(before)} → ${format(after)}).`;
  }
  ...
  function moveCharacter(id, destination) {
    ...
    if (m.assignments[destination].length >= MAX_PER_PARTY) {
      const el = $('#swapImpact');
  ...
  function install() {
    ...
    if (roster && !roster.dataset.partyRosterClick) {
      ...
      roster.addEventListener('click', (event) => {
        if (event.target.closest('a,.remove-character')) return;
        const heading = event.target.closest('h3');
        if (!heading) return;
        const card = heading.closest('.character');
    ...
    if (refresh && !refresh.dataset.authoritativeRefresh) {
      ...
      refresh.addEventListener('click', () => window.setTimeout(() => { correctClasses(); render(); makeRosterNamesClickable(); }, 100));
    }
