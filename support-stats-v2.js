...
const CAT={ap:/(^|support|attack|atk).*ap|attack.?power|support.?ap/i,brand:/brand/i,ha:/(^|support|h|has|haskill).*skill|(^|support)ha/i,identity:/identity/i};
...
const ALIASES={'horizon-cathedral-g1':['Archbishop Arcenos','Archbishop Arsenos','Horizon Cathedral Gate 1'],'horizon-cathedral-g2':['Arcenos, Vanguard of Fanaticism','Vanguard of Fanaticism','Horizon Cathedral Gate 2'],'serca-g1':['Witch of Agony, Serca','Witch of Agony Serca','Serca Gate 1','Serca'],'serca-g2':['Corvus Tul Rak','Corvus Tul Rat','Serca Gate 2','Serca'],'kazeros-g1':['Abyss Lord Kazeros','Kazeros Gate 1','Kazeros'],'kazeros-g2':['Death Incarnate Kazeros','Kazeros Gate 2','Kazeros'],'armoche-g1':['Brelshaza, Ember in the Ashes','Mistress of Desire Echidna','Covetous Master Echidna','Armoche Gate 1'],'armoche-g2':['Armoche, Sentinel of the Abyss','Armoche','Armoche Gate 2'],'extreme-aegir-g2':['Aegir, the Oppressor','Aegir','Extreme Aegir Gate 2'],'extreme-brelshaza-g2':['Phantom Manifester Brelshaza','Brelshaza','Extreme Brelshaza Gate 2']};
...
function categoryFrom(value,key){const k=norm(key),v=norm(value);for(const [cat,re] of Object.entries(CAT))if(re.test(k)||re.test(v))return cat;if(v.includes('supportap'))return'ap';if(v.includes('supportha')||v.includes('haskill'))return'ha';return''}
function numericMedian(v){if(v===null||v===undefined)return null;if(typeof v==='number')return Number.isFinite(v)?v:null;if(typeof v==='string'){const s=v.trim(),clean=s.replace(/%$/,'');if(!clean)return null;const n=Number(clean);return Number.isFinite(n)?(s.endsWith('%')?n/100:n):null}if(v&&typeof v==='object'){for(const k of ['median','med','value','uptime','fraction','percent','percentage']){const n=Number(v[k]);if(Number.isFinite(n))return(typeof v[k]==='string'&&v[k].trim().endsWith('%'))?n/100:n}}return null}
function collect(root){const out=[],seen=new WeakSet();function walk(v,ctxCategory='',ctxClass=''){if(!v||typeof v!=='object'||seen.has(v))return;seen.add(v);if(Array.isArray(v)){for(const x of v)walk(x,ctxCategory,ctxClass);return}const ownClass=classNorm(v.class||v.className||v.supportClass||v.supportClassName||v.roleClass||v.name)||ctxClass;const ownCategory=categoryFrom(v.stat,'stat')||categoryFrom(v.spec,'spec')||categoryFrom(v.metric,'metric')||categoryFrom(v.effect,'effect')||categoryFrom(v.type,'type')||categoryFrom(v.key,'key')||ctxCategory;const direct=numericMedian(v.median??v.med??v.value??v.uptime??v.fraction??v.percent??v.percentage);if(ownClass&&ownCategory&&direct!==null)out.push({class:ownClass,median:direct,category:ownCategory});for(const[k,x]of Object.entries(v)){const nextCategory=categoryFrom('',k)||categoryFrom(typeof x==='string'?x:'',k)||ownCategory;const nextClass=classNorm(k)||ownClass;const med=numericMedian(x);if(nextClass&&nextCategory&&med!==null)out.push({class:nextClass,median:med,category:nextCategory});walk(x,nextCategory,nextClass)}}walk(root);return out}
function merge(records){const result={};for(const s of SUPPORTS)result[s]={ap:null,brand:null,ha:null,identity:null};for(const r of records)if(result[r.class]&&r.category)result[r.class][r.category]=r.median;return result}
