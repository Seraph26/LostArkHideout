...
(()=>{
  ...
  let scheduled=false; function repair(){ if(scheduled)return; scheduled=true; requestAnimationFrame(()=>{scheduled=false;repairClassIcons();restorePartyOptimizer();}); }
  const observer=new MutationObserver(repair); function start(){repair();observer.observe(document.body,{childList:true,subtree:true});}
