...
function renderParty(title,p,score,opts){const solo=!!(opts&&opts.solo),key=(opts&&opts.key)||'party1';const model=window.LostArkEncounterScoring.profile();const fit=(window.LostArkEncounterScoring.partyScore(p).score*100).toFixed(1);return `<article class="party encounter-optimized-party${solo?' solo-encounter-party':''}"><h3>${esc(title)}</h3><div class="score">Encounter score ${Math.round(score).toLocaleString()} · Fit ${fit}%</div><div class="slots" data-enc-party="${key}">${p.map(c=>{const i=info(c),t=window.LostArkEncounterScoring.traits(c),r=window.LostArkEncounterScoring.characterScore(c),roleClass=i.role==='Support'?'support':'dps';return `<div class="slot party-member authoritative-member" draggable="true" data-character-id="${esc(c.id)}"><h4 class="party-character-title">${esc(i.name)}</h4><small>${esc(i.cls)} · <span class="party-role-label ${roleClass}">${esc(i.role)}</span> · CP ${Math.round(i.cp).toLocaleString()} · ${esc(t.positionLabel)} · Encounter ${Math.round(r.score*100)}%</small>${'<div class="encounter-hover"><strong>'+esc(i.name)+'</strong><div>'+esc(i.cls)+' · CP '+Math.round(i.cp).toLocaleString()+'</div><div>Encounter fit '+Math.round(r.score*100)+'% · '+esc(t.positionLabel)+'</div><div class="encounter-hover-reasons">'+esc(r.reasons.join(' · ')||'Neutral encounter profile')+'</div></div>'}</div>`}).join('')}</div></article>`}
