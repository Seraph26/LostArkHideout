...
(()=>{
  ...
  function repair(){
    try{
      ...
      let changed=false;
      for(const c of list){
        const cls=String(c?.profile?.class||c?.profile?.className||'').trim().toLowerCase();
        if(cls==='wildsoul' && c.profile.classIcon!==ICON){c.profile.classIcon=ICON;changed=true}
      }
      ...
      document.querySelectorAll('.new-addition-card[data-candidate-id]').forEach(card=>{
        ...
        if(String(c?.profile?.class||c?.profile?.className||'').trim().toLowerCase()==='wildsoul'){
          const img=card.querySelector('img.class-icon');
          if(img){img.src=ICON;img.alt='Wildsoul'}
        }
      });
    }catch{}
  }

  const previous=window.fetchCharacter;
  if(typeof previous==='function'&&!window.__WildsoulIconWrapped){
    window.__WildsoulIconWrapped=true;
    window.fetchCharacter=async function(candidate){
      const profile=await previous(candidate);
      if(profile&&String(profile.class||profile.className||'').trim().toLowerCase()==='wildsoul')profile.classIcon=ICON;
      return profile;
    };
  }

  window.WildsoulClassIcon=ICON;
