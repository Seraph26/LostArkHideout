...
function schedule(){[0,60,180,350,600,1000].forEach(ms=>setTimeout(()=>{if(!general()||optimizing||!swapBefore)return;if(sig()!==sig(swapBefore.state))render()},ms))}
function onOptimize(){if(!general())return;optimizing=true;swapBefore=null;clear();if(armTimer)clearTimeout(armTimer);armTimer=setTimeout(()=>{armTimer=null;/* arm() was never called, so this latch was never released: after an optimize
   click the top arrows were suppressed permanently. */optimizing=false},4500)}
...
function start(){const r=root()||document.body;document.getElementById('optimizeBtn')?.addEventListener('click',onOptimize,true);r.addEventListener('pointerdown',e=>{if(e.target.closest?.('.authoritative-member'))captureBefore()},true);r.addEventListener('dragstart',e=>{if(e.target.closest?.('.authoritative-member'))captureBefore()},true);/* Same as the metrics block: the optimizer's capture-phase drop handler on this
  ...
  document.addEventListener('drop',schedule,true);new MutationObserver(()=>{if(!general()||optimizing||!swapBefore)return;if(sig()!==sig(swapBefore.state))schedule()}).observe(r,{childList:true,subtree:true});document.getElementById('generalOptimization')?.addEventListener('change',()=>{if(!general()){optimizing=false;swapBefore=null;clear()}else{setTimeout(clear,50)}},true);setTimeout(clear,900)}
