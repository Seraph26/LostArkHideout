...
function render(){if(!active()||optimizing||!swapBefore)return;const now=potentials();if(!now)return;const title=swapNames(swapBefore,{state:domState()});place(now.p[0],arrow(now.p[0].v-swapBefore.p[0],swapBefore.p[0],title));place(now.p[1],arrow(now.p[1].v-swapBefore.p[1],swapBefore.p[1],title));place(now.c,arrow(now.c.v-swapBefore.c,swapBefore.c,title))}
...
function captureBefore(){if(!active()||optimizing||swapBefore)return;const p=potentials();if(!p)return;swapBefore={state:domState(),p:[p.p[0].v,p.p[1].v],c:p.c.v}}
