/* Lost Ark Hideout — support hover data bridge v4 */
(()=>{
'use strict';
const clean=s=>String(s??'').replace(/\s+/g,' ').trim();
const SUPPORTS=new Set(['Bard','Artist','Paladin','Valkyrie']);
function classFor(member){
  const explicit=clean(member.dataset.class||member.querySelector('[data-class]')?.dataset.class||member.querySelector('.class-icon')?.alt||'');
  if(SUPPORTS.has(explicit))return explicit;
  const text=clean(member.textContent||'');
...
function render(){
 document.querySelectorAll('#suggestedParties .party-member').forEach(member=>{
  ...
  const summary=summaryFor(member); const detail=member.querySelector('.character-hover-breakdown .chb-detail');
