/* Lost Ark Hideout — character hover explanations v6 */
(()=>{'use strict';
...
function humanizeEffects(text){let out=String(text??'');for(const[k,v]of Object.entries(LABELS))out=out.replace(new RegExp(`\\b${k}\\b`,'g'),v);return out.replace(/\bobserved median(?: uptime)?\b/gi,'observed median uptime')}
function formatEffectRow(row){
 const raw=clean(row.textContent);
 if(!raw||/^No direct party effects detected\.?$/i.test(raw))return;
 /* Never re-parse a row that this formatter already owns. This is critical because
    MutationObserver/hover events can run the formatter repeatedly. */
 if(/estimated contribution/i.test(raw))return;
...
function enhance(){document.querySelectorAll('.character-hover-breakdown').forEach(card=>{
 ...
 if(stats)[...stats.querySelectorAll('span')].forEach(span=>{const t=clean(span.textContent);if(/^Party Synergy\s+[+−-]/i.test(t)){const v=t.replace(/^Party Synergy\s*/i,'');if(!span.dataset.chbMetricDone){span.innerHTML=`<span class="chb-metric-label">Party Synergy</span> ${v}`;span.dataset.chbMetricDone='1'}span.title='Estimated increase to this character’s modeled potential from offensive synergies supplied by the other DPS characters in the party. This is a model percentage, not a direct in-game damage percentage.';span.classList.add('chb-explained-metric')}else if(/^Support\s+[+−-]/i.test(t)){const v=t.replace(/^Support\s*/i,'');if(!span.dataset.chbMetricDone){span.innerHTML=`<span class="chb-metric-label">Support Impact</span> ${v}`;span.dataset.chbMetricDone='1'}span.title='Estimated increase to this character’s modeled potential from the party support’s applicable buffs. This is a model percentage, not a direct in-game damage percentage.';span.classList.add('chb-explained-metric')}});
 ...
 /* Only operate on the dedicated summary element, never arbitrary nested div/span
    elements. This prevents parent hover-card text from being repeatedly rewritten. */
 card.querySelectorAll('.chb-synergy-summary').forEach(el=>{if(el.dataset.chbSynergySummaryDone==='1')return;const raw=clean(el.textContent);if(!/^Synergies\s*:/i.test(raw))return;el.textContent=humanizeEffects(raw).replace(/\s*·\s*/g,' · ');el.dataset.chbSynergySummaryDone='1';el.title='Synergies available from this party/encounter model. Support effects may also include observed median uptime data.';el.style.cursor='help'});
});}
function css(){let s=document.getElementById('character-hover-explanations-style');if(!s){s=document.createElement('style');s.id='character-hover-explanations-style';document.head.appendChild(s)}s.textContent='.chb-explained-metric{cursor:help;border-bottom:1px dotted rgba(255,255,255,.45)}.chb-explained-metric:hover{border-bottom-color:currentColor}.chb-metric-label{font-weight:600}.chb-synergy{display:block!important;margin:2px 0}.chb-synergy[title]{cursor:help}.chb-detail[title]{cursor:help}'}
